$(document).ready(function () {
 ...
 $.ajaxSetup({ async: false });
   ...
   /*  }); */
    ...
    var VentureID = document.getElementById('venture_id').value;//location_utl.searchParams.get("v");
    var CompID = document.getElementById('company_id').value;//location_utl.searchParams.get("c");
   
 if (projectid.indexOf(",") == -1) {
    console.log(VentureID);
        var pid = projectid;
        var dataurl = "https://localballot.in/mrrealtor/CompanyVentures/Getplots/"+VentureID+"/"+CompID;
        $.ajax({
            url: dataurl,
            type: 'GET',             
            dataType: 'json',
    ...
    $.ajax({
        url: "https://localballot.in/mrrealtor/CompanyVentures/projectgardenimages",
        type: 'GET',
    ...
    let url = "https://localballot.in/mrrealtor/CompanyVentures/getPlot_details/"+VentureID+"/"+CompID;
    console.log(url);
    $.ajax({
        url: url,
        type: 'GET',
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                var zoom = parseFloat(25);
                var minzoom = parseFloat(20);
                var maxzoom = parseFloat(30);
                ...
                var controls = ol.control.defaults({ rotate: false, attribution: false }).extend([
                ...
                var view = new ol.View({
                    center: [79.9792886611, 14.6019948855],
                    zoom: zoom,
                    minZoom: minzoom,
                    maxZoom: maxzoom,
					rotation: rotation
                    ...
                    if (feature) {
                        ...
                        if (zoom >= 17) {
                            ...
                            return new ol.style.Style({
                                ...
                                stroke: new ol.style.Stroke({ color: 'rgba(89,40,35,0.9)', width: 2 })
                            });
                        }
                        else {
                            return new ol.style.Style({
